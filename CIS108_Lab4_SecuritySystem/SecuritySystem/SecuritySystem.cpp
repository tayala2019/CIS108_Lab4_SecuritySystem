...
bool isPersonAllowed(std::string person)
{
	...
	for (int x = 0; x < (sizeof(ALLOW_ACCESS) / sizeof(char*)); x++)
	{
		...
		{
			is_allowed = true;   //( change from == to =)
			break;
		}
	}
...
void ssDoorLockEventHandler(DoorLock::DoorLockEvent event, std::string pin_code) // (convert int type to string)
{
	...
	{
		if (allowed_person_inside)   //( put a open bracket in if statement)
		{
	...
	{
		if (std::stoi(pin_code) == std::stoi(SECURE_PIN_CODE)) //(convert the string datatype on rightside into int by using std::stoi & same on left side)
		{
			access_granted = true;
			DoorLock::unlockDoor();
		}
...
void ssVideoDoorbellEventHandler(VideoDoorbell::VideoDoorbellEvent event, std::string person_at_door)
{
	...
	{
		MobileAppInterface::notifyFrontDoorActivity(false); // (put a semicolon)
	} break;
	}
}
